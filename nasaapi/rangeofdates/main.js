...
async function getPhotos(start, end) {
  ...
  try {
    ...
    document.querySelector(
      '#header'
    ).innerHTML = `Dates Ranging from ${start} through ${end}`;
            ...
            <div class="video">
               ${
                 mediaType === 'image'
                   ...
                   : videoUrl === 'youtube'
                   ? ` <iframe class="youtubeClass" width="550" height="400" src=${hdImage} frameborder="0" allowfullscreen></iframe>`
                   : videoUrl === 'vimeo'
                   ? `<iframe class="vimeoClass" src=${hdImage} width="550" height="400" frameborder="0"  allowfullscreen></iframe>`
                   : ''
               }
          </div>
       </div>
